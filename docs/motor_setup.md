...
```bash
candump -x can1 &                               # 응답을 계속 지켜봄. -x 가 RX/TX 를 구분해 줌
...
**`RX` 가 하나도 없으면 모터가 아무 답도 안 한 것임.** 전원, 종단저항 120Ω,
CANH/CANL 결선 순서로 봄.
...
**보낸 프레임과 자리가 다름.** 보낼 때는 맨 뒤가 대상 id 이고, 응답은 맨 뒤가 `FE`
이고 그 앞이 모터 id 임.
...
```
can1  TX  070AFD7F  [8]  00 00 00 00 00 00 00 00     <- 보낸 것. 에코임
...
| `RX` 로 뜨는 것 | 뜻 |
|---|---|
| `00000AFE` | 0x0A 로 바뀜. 성공 |
| 아무 RX 도 없음 | 안 바뀜. 옛 id 로 물어볼 것 |

안 오면 옛 id 로 물어봄.

```bash
cansend can1 0000FD7F#0000000000000000          # Type 0  옛 id(0x7F)에게
```

여기서 `00007FFE` 가 `RX` 로 오면 **변경이 안 된 것임.** 양쪽 다 답이 없으면 배선·전원
문제라 id 얘기가 아님.
...
|---|---|
| `00A` 3자리 | MIT 로 바뀜. 성공 |
| `0000 0AFE` 8자리 | 아직 private. 재투입을 안 했거나 Type 25 가 안 먹음 |
| 아무 RX 도 없음 | id·배선·전원. 3절의 Type 0 로 되짚음 |
...
| 보낸 것 | 정상 응답 | 무엇을 뜻하나 |
|---|---|---|
| `0000FD7F` | `00007FFE` | 그 모터의 id 가 0x7F 임 |
| `070AFD7F` | `00000AFE` | id 가 0x0A 로 바뀜 |
| `1200FD0A` | `00000AFE` | 파라미터 쓰기가 전달됨 |
| `1600FD0A` | 상태 프레임 | 저장 명령이 전달됨 |
| `1100FD0A` | `1100FD0A` + 값 | `Byte4` 가 읽은 값 |
| `1900FD0A` | `00000AFE` | 전환 명령이 전달됨. 재투입해야 적용 |
| `00A` (재투입 후) | `00A` + 상태값 | **MIT 로 바뀜** |
