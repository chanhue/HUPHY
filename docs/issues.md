...
지금은 순차 전송이라 프레임 6개 사이의 지연(약 0.8 ms)은 남음.

**수신 스레드는 붙였음** (`CanBus(channel, reader=True)`). 켜면 `recv` 대기가 제어
루프에서 빠지고 두 채널이 동시에 받음. `build_biped` 가 켬.

락 구성은 이렇게 정리됨 — 수신 스레드가 **송신 락을 잡지 않음.** 잡으면 `recv`
에서 블로킹하는 동안 송신이 밀려 스레드를 둔 이유가 사라짐. 공유하는 것은 프레임
`deque` 뿐이고 `append`/`popleft` 가 원자적이라 락이 필요 없음. 닫을 때는 스레드를
먼저 세우고 채널을 닫음.

남은 것: 실물 양다리에서 주기 예산을 재 보는 것. 12모터에서 목표 Hz 를 지키는지는
아직 확인 못 함.
