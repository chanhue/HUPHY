...
**저희 쪽 통신 어댑터(원격 조작·화면) 이야기는 여기 없습니다.** 그건 저희 저장소에 따로
있습니다. 이 문서는 **HUPHY 본체**에서 찾은 것만 담습니다 — 실제 모터와 주고받는 부분,
그리고 모터 2개짜리 축소 로봇을 만들 때 걸린 것들.
...
넣었습니다: 150도에서 경고, 170도에서 그 관절 명령 중단, 한 주기에 180도 이상 튀면
고정 차단. 다만 이건 저희를 거쳐 가는 명령에만 걸립니다.
...
저희(뷰어·원격 조작 쪽)가 2026-09-04 ~ 09-06 사이에 책상 위 모터 2개를 실제로 돌리며 찾은
것들입니다. 숫자는 전부 그 벤치에서 직접 잰 값이고, 추측은 추측이라고 적었습니다.

궁금한 점이나 재현이 필요하시면 저희 쪽에 말씀해 주십시오 — 같은 장비가 아직 그대로 있습니다.
