...
지금까지 쓰는 명령은 **11비트 ID 가 통째로 모터 CAN ID** 임 (`00A` = 모터 10).
그런데 매뉴얼 6.13~6.18절의 명령들은 **상위 3비트에 선택자**를 실음.

```
11-bit ID = (선택자 << 8) | 모터 CAN ID

선택자 0   지금 쓰는 명령 전부 (Enable/Stop/영점/고장/모드/ID/프로토콜/저장/능동보고)
       1   Command 10  위치 모드 제어
       2   Command 11  속도 모드 제어
       3   Command 14  파라미터 읽기
       4   Command 15  파라미터 쓰기
```

모터 CAN ID 가 1~127 이라 하위 8비트에 들어가므로 선택자와 겹치지 않음.

`_command_frame` 은 `can_id = motor_id` 만 만들어서 **선택자 명령을 못 보냄.**
전부 미구현임. 상수만 `SEL_*` 로 적어 둠.
...
인덱스도 값도 **작은 자리 먼저** 임 (상태 프레임과 반대 — `codec/mit.py` 참조).

**MIT 모드에서 파라미터를 못 만지는 것이 아님.** 읽고 쓰는 경로가 둘 있고 표는 같음.

| | 읽기 | 쓰기 (RAM) | 플래시 저장 |
|---|---|---|---|
| private | Type 17 | Type 18 | Type 22 |
| MIT | Command 14 (`SEL_PARAM_READ`) | Command 15 (`SEL_PARAM_WRITE`) | Command 12 (`CMD_SAVE`) |

둘 다 미구현임. 지금은 사람이 [`docs/motor_setup.md`](../../../../docs/motor_setup.md)
절차대로 private 에서 `cansend` 로 함. MIT 쪽은 펌웨어 요구사항이 있음
(RS00 `0.0.3.27` / RS02 `0.2.3.32` 이상).

MIT 파라미터 응답의 11비트 ID 는 `(3 << 8) | 모터ID` 라서 **상태 프레임(`0FD`)과
CAN ID 로 구분됨.** 고장 응답이 안 갈리는 문제가 여기엔 없음.
...
**응답** — Response Command 1 (매뉴얼 6.1절)
...
Byte6[5]              고장 있음
Byte6[4]              경고 있음
Byte6[3:0] + Byte7    권선 온도  12bit (0.1° 단위)
```

**명령과 응답의 배치가 다름** — 응답은 앞에 모터 ID가 붙어 한 칸씩 밀림.

**온도는 12비트임 ⚠️** Byte6 상위 4비트는 온도가 아님. 16비트로 읽으면 토크를
켠 순간(Motor 모드 → `Byte6[7]=1`) 3300°C 가 나옴. 토크가 꺼져 있을 땐 맞게 나와서
벤치에서는 안 보임. private 프로토콜에서는 이 플래그들이 29비트 CAN ID 안에 있어
`Byte6~7` 이 온전한 16비트 온도인데, MIT 은 11비트 ID 라 자리가 없어 내려왔음.
...
| `decode_flags(data)` | `StatusFlags(mode, fault, warning)` |
| `decode_fault(data)` | `(id, fault_word)`. `0`이면 정상 |

`decode_flags` 는 인코딩 범위를 안 받음 — 플래그는 양자화된 값이 아니라 생비트라
범위표가 틀려도 맞음. 그래서 `decode_state` 와 나눠 뒀음.
...
```
Byte0     모터 CAN ID
Byte1~4   고장값 32bit, 작은 자리 먼저 -- Byte1 이 최하위
```

**상태 프레임과 반대임.** 이 제조사는 프레임 종류마다 순서가 다름.

| | 순서 | 왜 |
|---|---|---|
| 제어 명령 · 상태 피드백 | 큰 자리 먼저 | 12비트 값이 바이트 경계를 넘나들어 손으로 조립한 것 |
| 파라미터 · 고장값 | **작은 자리 먼저** | MCU(ARM) 메모리의 32비트 값을 그대로 복사한 것 |

매뉴얼 6.8절 표에는 순서가 안 적혀 있지만 6.7절이 **"에러 코드는 응답의 BYTE1
자리로 돌아온다"** 고 함 — Byte1 이 최하위 바이트라는 뜻임. 고장값은 파라미터로도
읽히는 같은 레지스터(`faultSta`, uint32)이고, 파라미터는 매뉴얼이 작은 자리
먼저라고 명시함.

반대로 읽으면 **`raw` 는 0이 아닌데 이름이 하나도 안 붙음** — 과열(bit0)이
`0x01000000` 이 되어 어느 고장 비트에도 안 걸림. 이름 없는 16진수 줄이 나오면
이 순서를 의심할 것.

### `decode_fault`의 한계

고장 응답은 일반 상태 프레임과 **CAN ID가 같아**(둘 다 11비트 ID = 호스트 ID)
겉으로 구분되지 않음. 조회 명령을 보낸 직후의 첫 응답으로 간주해야 함.

**`decode_flags(data).fault` 가 더 나은 경로임.** 상태 프레임마다 딸려 오므로
왕복도 없고 오인할 여지도 없음. "무슨 고장인지"가 필요할 때만 `read_fault` 를 씀.
...
"읽기 명령" 은 없지만 **모터가 알아서 보내게** 할 수는 있음 — Command 13
(`CMD_ACTIVE_REPORT`, 매뉴얼 6.16절).

```
cansend can1 00A#FFFFFFFFFFFF01F9      # F_CMD=1 켬 / 0 끔(기본값)
```

켜면 Response Command 1 을 **기본 10ms 주기**로 스스로 보냄. 주기는 파라미터
`EPScan_time`(0x7026)으로 조정 — `1`=10ms, 이후 +1 마다 +5ms.

**제어 루프에는 쓰지 말 것.** 보고 시각이 모터 타이머 기준이라 명령과 안 붙음.
두 다리의 명령 시각을 맞추려고 전송을 몰아 둔 것([이슈 #10](../../../../docs/issues.md))이
무의미해짐. 명령을 안 보내면서 상태만 보고 싶은 자리 — 브링업 화면, 모니터링,
커미셔닝 중 관절을 손으로 돌려 보는 상황 — 에 맞음.

펌웨어 요구사항: RS00 `0.0.3.27` / RS02 `0.2.3.32` 이상.
