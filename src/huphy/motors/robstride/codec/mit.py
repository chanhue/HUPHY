...
def decode_state(data: bytes, *, enc: EncodingRange) -> Tuple[int, float, float, float, float]:
    ...
    # Byte6~7 은 온도 칸이지만 실측상 상위 4비트는 온도가 아님 (2026-09-05 벤치).
    #
    #   같은 모터·같은 순간에 두 경로가 다른 값을 냄:
    #     수신기 경로   0x814A = 33098 -> 3309.8 도
    #     커미셔닝 경로 0x014A =   330 ->   33.0 도
    #   그리고 0x214A(852.2 도) 도 관측됨. 세 값의 하위 12비트는 모두 0x14A = 330 = 33.0 도.
    #   다른 표본도 같음: 0x8168 -> 360 -> 36.0 도, 0x8140 -> 320 -> 32.0 도.
    #   하위 12비트만 온도로 읽으면 모든 관측이 방 온도로 설명되고, 상위 4비트를
    #   포함해서 읽으면 설명되지 않는 값만 나왔음. 12비트 x 0.1도 = 0~409.5 도라
    #   권선 온도 범위로도 충분함.
    #
    #   매뉴얼(p.37)은 "Byte6~7 권선 온도, 0.1도 단위"라고만 적고 상위 비트의 뜻은
    #   적지 않음. 그래서 여기서는 온도만 떼어 쓰고, 상위 4비트는 무엇인지 확인될
    #   때까지 버림. 이 마스크가 없으면 33도가 3309.8도로 보고되어, 과열 차단이
    #   헛돌거나(33도에서 차단) 진짜 과열을 놓침(130도가 13000도로 보여 "말이 안 되는
    #   값"으로 걸러짐).
    temp_u = ((data[6] << 8) | data[7]) & 0x0FFF
